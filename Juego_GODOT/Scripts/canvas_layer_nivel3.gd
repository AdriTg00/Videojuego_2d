...
func _ready():
	...
	score_label.text = "Score: " + str(Global.score_nivel3)  # Muestra "0" al inicio
...
func update_timer_label():
	var minutes = int(Global.tiempo_total_nivel3 / 60)
	var seconds = int(Global.tiempo_total_nivel3) % 60
	timerLabel.text = "%02d:%02d" % [minutes, seconds]
	timerLabel.text = "%02d:%02d" % [minutes, seconds]
	
func get_elapsed_ms() -> int:
	return int(round(Global.tiempo_total_nivel3 * 1000.0))  # INTEGER en SQLite
...
func añadir_moneda(amount: int):
	Global.score_nivel3 += amount
	score_label.text = "Score: " +str(Global.score_nivel3)
